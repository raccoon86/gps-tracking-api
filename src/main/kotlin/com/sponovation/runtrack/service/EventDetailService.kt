...
class EventDetailService(
        ...
        try {
            ...
            val testUsers = listOf(
                mapOf(
                    ...
                    "name" to "김러너",
                    "bibNumber" to "A001",
                    "profileImageUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=1"
                ),
                mapOf(
                    "userId" to 2L,
                    "name" to "이스피드",
                    "bibNumber" to "A002",
                    "profileImageUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=2"
                ),
                mapOf(
                    "userId" to 3L,
                    "name" to "박마라톤",
                    "bibNumber" to "A003",
                    "profileImageUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=3"
                ),
                mapOf(
                    "userId" to 4L,
                    "name" to "최러닝",
                    "bibNumber" to "A004",
                    "profileImageUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=4"
                ),
                mapOf(
                    "userId" to 5L,
                    "name" to "정트랙",
                    "bibNumber" to "A005",
                    "profileImageUrl" to "https://api.dicebear.com/7.x/avataaars/svg?seed=5"
                )
            )
    ...
    private fun getBatchParticipantLocations(
        ...
        logger.info(
            "통합 대상 참가자 실시간 위치 데이터 조회 시작: eventDetailId=$eventDetailId, " +
        ...
        logger.info(
            "통합 대상 참가자 실시간 위치 데이터 조회 완료: eventDetailId=$eventDetailId, " +
    ...
    private fun getParticipantLocationFromRedis(
        ...
        try {
            ...
            logger.debug(
                "Redis Hash 필드 수: key=$key, userId=$userId, 필드 수=${allFields.size}, " +
            ...
            logger.debug(
                "Redis Hash 필드 수: key=$key, userId=$extractedUserId, 필드 수=${allFields.size}, " +
                        "필드 목록=${allFields.keys}"
            )
            ...
            if (correctedLatitude == null) {
                logger.warn(
                    "필수 필드 누락/파싱 실패 - correctedLatitude: key=$key, userId=$userId, " +
            ...
            if (correctedLongitude == null) {
                logger.warn(
                    "필수 필드 누락/파싱 실패 - correctedLongitude: key=$key, userId=$userId, " +
            ...
            // 원본 GPS 데이터 추출
            val rawAltitude = allFields["rawAltitude"]?.toString()?.takeIf { it != "null" }?.toDoubleOrNull()
            val rawSpeed = allFields["rawSpeed"]?.toString()?.takeIf { it != "null" }?.toFloatOrNull()

            // Participant 정보 조회
            val participant = try {
                participantRepository.findByEventDetailIdAndUserId(eventDetailId, userIdLong ?: 0L)
            } catch (e: Exception) {
                logger.warn("참가자 정보 조회 실패: userId=${userIdLong}, eventDetailId=$eventDetailId", e)
                null
            }

            logger.debug(
                "Redis GPS 데이터 + 참가자 정보 추출 완료: key=$key, userId=$userId, " +
                        "보정위치=($correctedLatitude, $correctedLongitude), " +
                        "보정고도=$correctedAltitude, 원본고도=$rawAltitude, " +
                        "원본속도=$rawSpeed, 참가자명=${participant?.name}"
            )

            return EventParticipantLocationDto(
                userId = userIdLong ?: 0L,
                name = participant?.name ?: "참가자_${userIdLong}",
                profileUrl = participant?.profileImageUrl,
                bibNumber = participant?.bibNumber,
                latitude = correctedLatitude ?: 0.0,
                longitude = correctedLongitude ?: 0.0,
    ...
    private fun getLocationFromRedis(key: String): EventParticipantLocationDto? {
        try {
            ...
            // Participant 정보 조회 (eventDetailId는 알 수 없으므로 기본값 사용)
            val participant = try {
                // key에서 eventDetailId 추출: location:{userId}:{eventDetailId}
                val keyParts = key.split(":")
                val eventDetailIdFromKey = if (keyParts.size >= 3) keyParts[2].toLongOrNull() else null
                if (eventDetailIdFromKey != null) {
                    participantRepository.findByEventDetailIdAndUserId(eventDetailIdFromKey, userId)
                } else null
            } catch (e: Exception) {
                logger.warn("참가자 정보 조회 실패 (하위 호환성): userId=$userId, key=$key", e)
                null
            }

            return EventParticipantLocationDto(
                userId = userId,
                name = participant?.name ?: "참가자_${userId}",
                profileUrl = participant?.profileImageUrl,
                bibNumber = participant?.bibNumber,
                latitude = correctedLatitude,
                longitude = correctedLongitude,
                altitude = allFields["correctedAltitude"]?.toString()?.toDoubleOrNull(),
                speed = rawSpeed,
            )
     ...
     * @return 코스 정보
     */
    private fun getCourseInfoAndGpxUrl(
        eventId: Long,
        eventDetailId: Long
    ): com.sponovation.runtrack.domain.EventDetail {
        try {
    ...
    private fun buildMapMarkerData(
        ...
        logger.info(
            "지도 마커 데이터 구성 시작: eventId=$eventId, eventDetailId=$eventDetailId, " +
        ...
        try {
            ...
            logger.info(
                "마커 우선순위 분류: eventDetailId=$eventDetailId, " +
            ...
            logger.info(
                "지도 마커 데이터 구성 완료: eventId=$eventId, eventDetailId=$eventDetailId, " +
            ...
            sortedMarkerData.forEachIndexed { index, marker ->
                getMarkerType(marker.userId, top3UserIds, currentUserId, trackedUserIds)
                getMarkerPriority(marker.userId, top3UserIds, currentUserId, trackedUserIds)
            }
    ...
    private fun getTargetUserIds(eventId: Long, eventDetailId: Long, currentUserId: String?): List<String> {
        ...
        try {
            ...
            currentUserId?.let { userId ->
                ...
                logger.info(
                    "트래커 목록 사용자 추가 완료: currentUserId=$userId, eventId=$eventId, " +
            ...
            logger.info(
                "위치 조회 대상 userId 통합 완료: eventId=$eventId, eventDetailId=$eventDetailId, " +
    ...
    private fun getTrackedUserIds(currentUserId: String, eventId: Long): List<String> {
        ...
        return try {
            ...
            logger.info(
                "트래킹 목록 조회 완료: currentUserId=$currentUserId, eventId=$eventId, " +
