...
class GpxService(
            ...
            checkpoints.forEach { checkpoint ->
                ...
                if (distanceToCheckpoint <= CHECKPOINT_CROSSING_THRESHOLD) {
                    ...
                    if (!alreadyPassed) {
                        ...
                        if (crossedBoundary) {
                            logger.info("새로운 체크포인트 통과 감지: cpId=$cpId, cpIndex=$cpIndex, " +
    ...
    private fun saveCheckpointCrossings(
        ...
        crossings.forEach { crossing ->
            try {
                ...
                logger.info("체크포인트 통과 기록 저장 완료: userId=$userId, cpId=${crossing.cpId}, " +
        ...
        return try {
            ...
            val cumulativeTime = if (segmentDuration != null) {
                if (currentIndex <= 0) {
                    ...
                    val previousCumulativeTime = getPreviousCumulativeTime(userId, eventId, eventDetailId, currentIndex - 1, sortedPassTimes)
    ...
    private fun saveSegmentRecord(
        ...
        try {
            ...
            logger.debug("구간 기록 저장 완료: userId=$userId, checkpointId=$checkpointId, " +
    ...
    fun correctLocation(request: CorrectLocationRequestDto): CorrectLocationResponseDto {
        ...
        try {
            ...
            gpsDataList.forEachIndexed { index, gpsData ->
                ...
                logger.debug("3차원 칼만 필터 적용: 원본=($lat, $lng, $alt) -> " +
                    "필터링=(${"%.6f".format(filtered.first)}, ${"%.6f".format(filtered.second)}, " +
                    "${filtered.third?.let { "%.2f".format(it) } ?: "null"})")
                logger.debug("GPS 정확도: ${accuracy}m, 신뢰도: ${if (accuracy != null) calculateGpsConfidence(accuracy, gpsData.speed) else "기본값"}")
                    ...
                    if (index == gpsDataList.size - 1) {
                        ...
                        logger.info("최종 맵매칭 완료: 보정위치=(${"%.6f".format(finalCorrectedLat)}, ${"%.6f".format(finalCorrectedLng)}, " +
            ...
            logger.info("위치 보정 완료: ${gpsDataList.size}개 포인트 처리, " +
            ...
            gpsDataList.forEach { gpsData ->
                if (gpsData.accuracy != null && gpsData.accuracy > 0) {
                    ...
                    kalmanForFinal.filterWithWeights(gpsData.lat, gpsData.lng, gpsData.altitude, gpsData.accuracy, confidence)
                } else {
                    kalmanForFinal.filter(gpsData.lat, gpsData.lng, gpsData.altitude)
                }
            }
            val filtered3D = kalmanForFinal.getCurrentPosition3D()
            val filtered = if (filtered3D != null) Pair(filtered3D.first, filtered3D.second) else Pair(lastGpsData.lat, lastGpsData.lng)
    ...
    private fun updateCorrectedLocationInRedis(
        ...
        try {
            ...
            logger.info("보정 위치 Redis 업데이트 완료: userId=$userId, eventDetailId=$eventDetailId, " +
    ...
    private fun updateLeaderboardSortedSet(
        ...
        try {
            ...
            logger.info("리더보드 Sorted Set 업데이트: eventDetailId=$eventDetailId, userId=$userId, " +
